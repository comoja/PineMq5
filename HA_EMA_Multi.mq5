...
input int InpTrailDivisions = 6; // Partes para avance del SL dinamico
...
void OnTick()
{
    ...
    if(currentBarTime == lastBarTime)
    {
      // --- PERSECUCIÓN DE TAKE PROFIT (TP CHASING) EN TIEMPO REAL ---
        if(PositionSelect(Symbol()) && PositionGetInteger(POSITION_MAGIC) == MagicNumber && useTPChase)
        {
            ...
            double chaseDistance = tpChasePts * point;
            double chaseOffset = tpChaseOffset * point;
            ...
            if(currentTP > 0)
            {
                if(type == POSITION_TYPE_BUY)
                {
                    ...
                    if((currentTP - highCurr) <= chaseDistance)
                    {
                        double newTP = NormalizeDouble(highCurr + MathMax(chaseOffset, stopsDistance), digits);
                        if(newTP > currentTP && trade.PositionModify(Symbol(), currentSL, newTP))
                        {
                            ...
                            if(!tpChaseTriggered)
                            {
                                ...
                                double maxValidSL = highCurr - stopsDistance;
                                double newSL = NormalizeDouble(MathMin(halfTpSL, maxValidSL), digits);
                                if(newSL > currentSL && trade.PositionModify(Symbol(), newSL, newTP))
                                {
                                    activeSL = newSL;
                                    currentSL = newSL;
                                }
                    ...
                    if(tpChaseTriggered && tpChaseSlGap > 0.0)
                    {
                        double trailingSL = NormalizeDouble(MathMin(highCurr - tpChaseSlGap, highCurr - stopsDistance), digits);
                ...
                else if(type == POSITION_TYPE_SELL)
                {
                    ...
                    if((lowCurr - currentTP) <= chaseDistance)
                    {
                        double newTP = NormalizeDouble(lowCurr - MathMax(chaseOffset, stopsDistance), digits);
                        if(newTP < currentTP && trade.PositionModify(Symbol(), currentSL, newTP))
                        {
                            activeTP = newTP;
                            if(!tpChaseTriggered)
                            {
                                double halfTpSL = positionEntry - (MathAbs(newTP - positionEntry) / 2.0);
                                double minValidSL = lowCurr + stopsDistance;
                                double newSL = NormalizeDouble(MathMax(halfTpSL, minValidSL), digits);
                                if((newSL < currentSL || currentSL == 0.0) && trade.PositionModify(Symbol(), newSL, newTP))
                                {
                                    activeSL = newSL;
                                    currentSL = newSL;
                                }
                    ...
                    if(tpChaseTriggered && tpChaseSlGap > 0.0)
                    {
                        double trailingSL = NormalizeDouble(MathMax(lowCurr + tpChaseSlGap, lowCurr + stopsDistance), digits);
   ...
   // ============================================================================
    if(!outOfMarket)
    {
        ...
        double stopsDistance = MathMax((double)SymbolInfoInteger(Symbol(), SYMBOL_TRADE_STOPS_LEVEL), minStopsLevel) * point;
        activeTP = currentTP;
        double tpDistance = MathAbs(activeTP - entryP);
        if(activeTP > 0.0)
        {
            trailStep = tpDistance / (double)trailDivisions;
        }
      ...
      // --- SL DINAMICO DESPUES DE ACTIVARSE EL TP CHASING ---
        if(tpChaseTriggered && tpChaseSlGap > 0.0)
        {
            if(inLong)
            {
                double trailingSL = NormalizeDouble(MathMin(bid - tpChaseSlGap, bid - stopsDistance), digits);
            ...
            {
                double trailingSL = NormalizeDouble(MathMax(ask + tpChaseSlGap, ask + stopsDistance), digits);
      ...
      // --- TRAILING STOP POR PARTES DEL TAKE PROFIT ---
        if(!tpChaseTriggered && activeTP > 0.0 && trailStep > 0.0 && inLong)
        {
            double recorrido = bid - entryP;
        ...
        else if(!tpChaseTriggered && activeTP > 0.0 && trailStep > 0.0 && inShort)
        {
            double recorrido = entryP - ask;
            int niveles = (int)MathFloor(recorrido / trailStep);
            if(niveles > 0)
   ...
   // ============================================================================
    ...
    double candleOpen = iOpen(_Symbol, PERIOD_CURRENT, 1);
    double candleClose = iClose(_Symbol, PERIOD_CURRENT, 1);
   ...
   // Filtro de spread de EMAs
    ...
    bool strongBull = candleGreen && body >= atrVal * 0.10;
    bool strongBear = candleRed && body >= atrVal * 0.10;
